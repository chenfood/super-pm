...
我会带你走完一套完整的交互设计流程，最终输出可在浏览器中体验的交互原型。

**流程：** P1 用户画像 → P2 设计语言（浏览器选择）→ P3 任务流+情感地图 → P4 多套方案对比 → P5 可交互原型（手机外壳）→ P6 交付/开发

**准备：** 有个大致想法就行，细节我逐步引导。P2 起会启动浏览器预览服务。版本升级可复用之前的画像和设计语言。
...
- [ ] **P5 Prototype** — 中保真原型（手机外壳 + iPhone/Android 切换），确认后保存到 `.super-pm/artifacts/{feature}/` → 详见 [prototype-templates.md](references/prototype-templates.md)
